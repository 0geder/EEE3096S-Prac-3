...
volatile uint64_t checksum_float;
/* USER CODE END PV */
...
int main(void)
{
  ...
  {
    // Run the most computationally intensive benchmark continuously
    checksum_float = calculate_mandelbrot_float(IMAGE_SIZE, IMAGE_SIZE, MAX_ITER);

    // NOTE: In a real measurement, you might remove the below section
    // and just let it loop on the calculation. For this practical, we'll
    // break out of this loop after one run to demonstrate the idle state.
    break;
  }
  ...
  while (1)
  {
    ...
    // A more advanced implementation might use a `__WFI()` (Wait For Interrupt)
    // instruction to enter a low-power sleep mode.
  }
